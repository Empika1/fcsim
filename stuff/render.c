...
int main(void)
{
    ...
    struct fcsim_block_stat* block_stats = malloc(the_arena_layer.arena.block_cnt * sizeof(struct fcsim_block_stat));
    ...
    do
    {
        ...
        draw_arena(&the_arena_layer.arena, block_stats);

        glfwSwapBuffers(window);
        glfwPollEvents();
    } while (!fcsim_has_won(&the_arena_layer.arena, block_stats));
    ticks -= 1;
